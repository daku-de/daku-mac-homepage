function sendMail() {
    ...
    if (subject == "") {
        document
            .getElementById("mail-subject")
            .style.setProperty("border", "1px solid #FF2C2C");
        invalid = true;
    }
    if (message == "") {
        document
            .getElementById("mail-message")
            .style.setProperty("border", "1px solid #FF2C2C");
        invalid = true;
    }
        ...
        if (req.readyState == XMLHttpRequest.DONE) {
            if (req.status == 200) {
                ...
                document
                    ...
                    .style.setProperty("animation-name", "send-mail-success");
                setTimeout(
                    () =>
                        document
                            .getElementById("send-mail")
                ...
                document
                    .getElementById("send-mail")
                    .style.setProperty("animation-name", "send-mail-failure");
                setTimeout(
                    () =>
                        document
                            .getElementById("send-mail")
...
$(document).ready(function (e) {
    ...
    function printTerminalWelcome() {
        ...
        printLine();
        printLine("Welcome to [^https://daku.de](<b>daku.de</b>)!");
        printLine();
        printLine(
        ...
        switch (command) {
            ...
            case "wiki":
                ...
                let search_url =
                    ...
                    wiki_title +
                    "&limit=1&format=json&explaintext&origin=*";
                ...
                x.onreadystatechange = function () {
                    if (x.readyState == XMLHttpRequest.DONE) {
                        ...
                        let url =
                            "https://en.wikipedia.org/w/api.php?action=query&prop=extracts&explaintext&format=json&titles=" +
                            wiki_page +
                            "&redirects&origin=*";
                        ...
                        req.onreadystatechange = function () {
                            if (req.readyState == XMLHttpRequest.DONE) {
                                ...
                                let extract =
                                    wiki_info.query.pages[
                                        Object.keys(wiki_info.query.pages)[0]
                                    ].extract;
                                let pageid =
                                    wiki_info.query.pages[
                                        Object.keys(wiki_info.query.pages)[0]
                                ...
                                if (
                                    extract.includes("refer to:") ||
                                    extract.includes(
                                        "nickname of the following people:",
                                    )
                                ) {
                                    if (
                                        extract.includes(
                                    ...
                                    printLine();
                                    printLine(
                                        '<span style="font-size: 1.5em; font-weight: bold;"><a href="' +
                                            "http://en.wikipedia.org/wiki?curid=" +
                                            pageid +
                                            '" target="_blank">' +
                                            wiki_info.query.pages[
                                                Object.keys(
                                ...
                                let desc = extract.substr(
                                    0,
                                    extract.indexOf("<br><br><br>"),
                                );
                                printLine();
                                printLine(
                                    '<span style="font-size: 1.5em; font-weight: bold;"><a href="' +
            ...
            case "radio":
                if (args.length > 1) {
                    printLine(
                        "Usage: radio &lt;volume&gt;. The value has to be between 0 and 100. Current volume is " +
                            radio.volume * 1000 +
                            ".",
                    );
                    break;
                ...
                if (args.length == 1) {
                    ...
                    if (!radioPlaying) {
                        ...
                        printLine(
                            "To change the volume use 'radio &lt;volume&gt;'. The value has to be between 0 and 100. Current volume is " +
                                radio.volume * 1000 +
                                ".",
                        );
                    } else {
            ...
            case "guestbook":
                ...
                if (args.length == 0) {
                    ...
                    if (!name.match(/^[\w\d]{3,10}( [\w\d]{3,10})?$/g)) {
                        printLine(
                            "This name is invalid, your name can consist of up to two words, each word has to be between 3 and 10 letters / digits long.",
                        );
                        break;
                    }
                    req.open("POST", url);
                        ...
                        if (req.readyState == XMLHttpRequest.DONE) {
                            if (req.status == 200) {
                                ...
                                if (req.responseText == "DONE")
                                    printLine(
                                        "Your name was added to the guest book!",
                                    );
                                if (req.responseText == "NAME ALREADY USED")
                                    printLine(
                                        "This name is already registered in the guest book!",
                                    );
                            }
                        }
                    };
            ...
            case "style":
                if (args.length == 1) {
                    ...
                    Object.keys(terminalstyles).forEach((style) => {
                        printLine(style);
                    });
                }
                break;
            ...
            case "cd":
                if (args.length == 1) {
                    ...
                    if (folder == "..") {
                        ...
                        dir = dir.getParent();
                        printLine(
                            "Changed directory to: " + dir.getDirectory(),
                        );
                        break;
                    }
                    if (folder == ".") break;
                    if (
                        dir.getChild(folder) != null &&
                        dir.getChild(folder) instanceof Folder
                    ) {
                        ...
                        printLine(
                            "There is no folder called '" + folder + "'!",
                        );
                    }
                } else {
            ...
            case "cat":
                if (args.length == 1) {
                    let file = args[0];
                    if (
                        dir.getChild(file) != null &&
                        dir.getChild(file) instanceof Textfile
                    ) {
                        printLine(dir.getChild(file).getContent());
                    } else {
                        printLine(
                            "There is no text file called '" + file + "'!",
                        );
                    }
                } else {
            ...
            case "create":
                if (args.length > 1) {
                    ...
                    let content = line.substr(
                        line.indexOf(file) + file.length,
                        line.length,
                    );
                    content = content.substr(
                        content.indexOf(args[1]),
                        content.length,
                    );
    ...
    function help(arg) {
        function printCommandlist(commands) {
            ...
            for (let i = 0; i < commands.length; i++) {
                let e = commands[i];
                if (e[0].length + e[2].length >= maxlen)
                    maxlen = e[0].length + e[2].length;
            ...
            cmd_table +=
                "│ " +
                "\u00A0".repeat(Math.floor((maxlen - "Command".length) / 2)) +
                "<b>Command</b>" +
                "\u00A0".repeat(
                    Math.ceil((maxlen - "Command".length) / 2) + 1,
                ) +
                " │ ";
            cmd_table +=
                "\u00A0".repeat(
                    Math.floor((maxlen_desc - "Description".length) / 2),
                ) +
                "<b>Description</b>" +
                "\u00A0".repeat(
                    Math.ceil((maxlen_desc - "Description".length) / 2),
                ) +
                " │" +
                "<br>";
            cmd_table += box_mid;
            for (let i = 0; i < commands.length; i++) {
                ...
                let output =
                    "│ " +
                    "\u00A0".repeat(
                        Math.floor((maxlen - (e[0].length + e[2].length)) / 2),
                    ) +
                    e[0] +
                    "\u00A0" +
                    e[2];
                output +=
                    "\u00A0".repeat(
                        Math.ceil((maxlen - (e[0].length + e[2].length)) / 2),
                    ) +
                    " │ " +
                    "\u00A0".repeat(
                        Math.floor((maxlen_desc - e[1].length) / 2),
                    ) +
                    e[1] +
                    "\u00A0".repeat(
                        Math.ceil((maxlen_desc - e[1].length) / 2),
                    ) +
                    " │";
        ...
        switch (arg) {
            ...
            case "shell":
                printLine();
                printLine(
                    "HELP - " + arg.charAt(0).toUpperCase() + arg.slice(1),
                );
                printLine();
            ...
            case "features":
                ...
                printLine();
                printLine(
                    "This page offers you different functionalities. E.g. you can use a calculator.",
                );
                printLine("");
                printLine();
                printCommandlist(commandlist[arg]);
            ...
            case "layout":
                ...
                printLine(
                    "You can change the wallpaper as well as the style of the shell.",
                );
                printLine();
                printCommandlist(commandlist[arg]);
                break;
            case "filesystem":
                ...
                printLine(
                    "To change your current directory you can go up by using 'cd ..' or down by using 'cd folder'.",
                );
                printLine();
                printCommandlist(commandlist[arg]);
                break;
            default:
                ...
                printLine(
                    "There is no help for '" +
                        arg.charAt(0).toUpperCase() +
                        arg.slice(1) +
                        "'!",
                );
                printLine(
                    "You can type 'help [topic]' for these topics: " + topics,
                );
                printLine();
        }
    }
    ...
    function parseURL(content) {
        while (content.indexOf("](") >= 0) {
            ...
            if (newpage) {
                content = content
                    .replace(content.substring(start, end + 1), "")
                    .splice(
                        start,
                        0,
                        '<a href="' +
                            url +
                            '" target="_blank" tabindex="-1"><b>' +
                            uname +
                            "</b></a>",
                    );
            } else {
    ...
    function printLine(content, style, service, servicestyle) {
        ...
        if (service == null) {
            stream.append(
                '<div class="line">' +
                    '<p class="information" style="color:var(--color-' +
                    style +
                    ')">' +
                    content +
                    "</p>" +
        ...
        stream.append(
            '<div class="line">' +
                ...
                seconds +
                "]</p>" +
                '<p class="name" style="color:var(--color-' +
                servicestyle +
                ')">' +
                service +
                "</p>" +
                '<p class="content" style="color:var(--color-' +
                style +
                ')">' +
                content +
                "</p>" +
    ...
    function calc() {
        $(".num").click(function () {
            ...
            document.getElementById("calc-input").scrollLeft =
                document.getElementById("calc-input").scrollWidth;
        });
        $(".op").click(function () {
            if (calc_result != "") {
